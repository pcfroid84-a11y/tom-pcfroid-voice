...
const SMALL_FRENCH = ["zero","un","deux","trois","quatre","cinq","six","sept","huit","neuf","dix","onze","douze","treize","quatorze","quinze","seize"];
...
for (let number = 0; number < 100; number += 1) UNDER_100.set(frenchUnder100(number), number);

function cleanNumberTokens(tokens) {
  return tokens.map((token) => token === "vingts" ? "vingt" : token === "cents" ? "cent" : token).filter(Boolean);
}
...
const NUMBER_VOCABULARY = new Set([...SMALL_FRENCH,"vingt","vingts","trente","quarante","cinquante","soixante","cent","cents","mille","et"]);
function numericWordRuns(text) {
  const tokens = normalizeNumberText(text).split(" ").filter(Boolean);
  const runs = [];
  let current = [];
  for (const token of tokens) {
    if (NUMBER_VOCABULARY.has(token)) current.push(token);
...
function postalFromSpokenRun(run) {
  ...
  if (cleaned.length === 5 && cleaned.every((token) => DIGIT_WORDS.has(token))) return cleaned.map((token) => DIGIT_WORDS.get(token)).join("");
  if (cleaned.includes("mille")) {
    const cardinal = parseFrenchCardinal(cleaned);
    if (cardinal != null && cardinal >= 0 && cardinal <= 99999) return String(cardinal).padStart(5, "0");
  }
  ...
  for (let split = cleaned.length - 1; split >= 1; split -= 1) {
    ...
    if (department == null || suffix == null || department < 0 || department > 99 || suffix < 0 || suffix > 999) continue;
...
export function extractPostalCode(text = "") {
  ...
  if (numericGroups) {
    ...
    if (department >= 0 && department <= 99 && suffix >= 0 && suffix <= 999) return `${String(department).padStart(2, "0")}${String(suffix).padStart(3, "0")}`;
  }
  ...
  if (leadingDigits) {
    ...
    if (department >= 0 && department <= 99 && suffix != null) return `${String(department).padStart(2, "0")}${String(suffix).padStart(3, "0")}`;
  }
  ...
  if (trailingDigits) {
    ...
    if (department != null && suffix >= 0 && suffix <= 999) return `${String(department).padStart(2, "0")}${String(suffix).padStart(3, "0")}`;
  }
...
function decisionFromEntry(entry, service) {
  ...
  return { status, known: true, city: entry.city, postalCodes: [...(entry.postal_codes || [])], candidates: [entry.city], source: "city" };
}

export function classifySectorRequest({ city, postalCode, service = "depannage", existingCustomer = false } = {}) {
  if (existingCustomer) return { status: "review", known: true, city: city || null, postalCodes: postalCode ? [String(postalCode)] : [], candidates: city ? [city] : [], source: "existing-customer-override" };
  ...
  if (postal.length === 5) {
    ...
    if (matches.length === 1) return { ...decisionFromEntry(matches[0], service), source: "postal" };
    if (matches.length > 1) {
      const statuses = [...new Set(matches.map((entry) => entry?.[service] || "review"))];
      return { status: statuses.length === 1 ? statuses[0] : "review", known: true, city: null, postalCodes: [postal], candidates: matches.map((entry) => entry.city), source: "postal-multiple" };
    }
    const departmentPrefix = postal.slice(0, 2);
    const nearDepartments = new Set(["84", "13", "30"]);
    return { status: nearDepartments.has(departmentPrefix) ? "review" : "no", known: false, city: null, postalCodes: [postal], candidates: [], source: nearDepartments.has(departmentPrefix) ? "postal-near-department-unknown" : "postal-outside-department" };
  }
  return { status: "unknown", known: false, city: null, postalCodes: [], candidates: [], source: "unknown-city" };
}
...
export function getSectorData() { return sector; }
